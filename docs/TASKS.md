...
- **PublicSurplus markup — resolved for card parsing, open for category.** Verified 2026-09-03 against a real captured search-results page on the mobile subdomain (`m.publicsurplus.com`): real query params (`keyWord`, 0-indexed `page`, `zipCode`/`milesLocation`) and real card selectors are in `poller/adapters/publicsurplus.py`. Still open: `category` isn't wired up (PublicSurplus categories are numeric `catId`s and none has been captured), and pagination past page 0 is unconfirmed.
