...
Outbound network access to govdeals.com was blocked in the environment this
adapter was developed in, so the CSS selectors and URL parameters below are a
best-effort guess at GovDeals's search-results markup, not something verified
against the live site. Everything around the parsing (URL building,
pagination, rate limiting, client-side filtering, error handling) is real and
unit-tested against the synthetic fixture at
tests/fixtures/govdeals/search_results.html.

Before trusting this adapter's results:
1. Confirm GovDeals search results are still server-rendered HTML (per FR6).
   If the current site requires JS to render results, this requests+BS4
   approach won't work and the site should be dropped from v1 or a JSON API
   endpoint the page calls should be used instead.
2. Run `python -m poller search govdeals "<a keyword you expect matches for>"`
   against the live site, save the HTML of a results page, and update
   SEARCH_URL / _ROW_SELECTOR / _parse_row (and the fixture, so the tests
   keep covering the real structure) to match what you see.
"""
...
SITE_NAME = "govdeals"
SEARCH_URL = "https://www.govdeals.com/search"
DEFAULT_PAGE_CAP = 3

# TODO(unverified): confirm against a live results page.
_ROW_SELECTOR = "div.listing-card"
_TITLE_SELECTOR = "a.listing-title"
_PRICE_SELECTOR = ".current-bid"
_BID_COUNT_SELECTOR = ".bid-count"
_END_TIME_SELECTOR = ".time-left"
_THUMB_SELECTOR = "img.listing-thumb"
...
def _parse_row(row) -> Listing | None:
    ...
    url = href if href.startswith("http") else f"https://www.govdeals.com{href}"
